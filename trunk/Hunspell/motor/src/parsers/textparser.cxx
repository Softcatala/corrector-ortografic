...
TextParser::TextParser(struct unicode_info2 * uconv, unsigned short * wordchars, int len)
{
...
int TextParser::is_wordchar(char * w)
{
	...
	if (utf8) {
		...
		u8_u16(&wc, 1, w);
                idx = (wc.h << 8) + wc.l;
                return (utfconv[idx].cletter || (wordchars_utf16 && flag_bsearch(wordchars_utf16, *((unsigned short *) &wc), wclen)));
        } else {
...
void TextParser::init(struct unicode_info2 * uconv, unsigned short * wc, int len)
{
	for (int i = 0; i < MAXPREVLINE; i++) {
		line[i][0] = '\0';
	}
